    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 space-y-8 md:space-y-0">
        <div className="max-w-xl">
          <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4">Browse Archive</h1>
          <p className="text-brand-ink/60 text-lg">
            Discover the depth and richness of the Yorùbá lexicon. Search by word, meaning, or browse alphabetically.
          </p>
        </div>
        ...
        <div className="flex space-x-2 min-w-max">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
              selectedLetter === null ? 'bg-brand-orange text-white' : 'bg-white/50 text-brand-ink/60 hover:bg-white'
            }`}
          >
            All
          </button>
          {alphabet.map((letter) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                selectedLetter === letter ? 'bg-brand-orange text-white' : 'bg-white/50 text-brand-ink/60 hover:bg-white'
              }`}
            >
              {letter}
            </button>
        ...
        <AnimatePresence mode="popLayout">
          {loading ? (
            Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-64 rounded-2xl bg-white/30 animate-pulse border border-brand-ink/5" />
            ))
            ...
            <div className="col-span-full py-24 text-center">
              <BookOpen size={48} className="mx-auto text-brand-ink/10 mb-6" />
              <h3 className="text-2xl font-serif font-bold mb-2">No entries found</h3>
              <p className="text-brand-ink/60">Try adjusting your search or filter criteria.</p>
            </div>
          )}
        </AnimatePresence>
      </div>
  ...
  return (
    <div 
      className={`glass-card p-8 rounded-2xl transition-all duration-300 cursor-pointer group hover:border-brand-orange/30 ${
        isExpanded ? 'ring-2 ring-brand-orange/20 shadow-2xl' : 'hover:shadow-lg'
      }`}
    ...
    >
      ...
      <p className={`text-brand-ink/70 leading-relaxed mb-6 ${isExpanded ? '' : 'line-clamp-2'}`}>
        {entry.definition}
      </p>
          ...
          >
            <div className="space-y-6">
              {entry.example_yoruba && (
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 mb-2">Example (Yorùbá)</h4>
              ...
              {entry.example_english && (
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 mb-2">Translation (English)</h4>
