...
import { ShieldCheck, CheckCircle, Clock, AlertCircle, LayoutDashboard, BookA, Layers, Edit3, Trash2, ChevronDown } from "lucide-react";
...
export const Admin = () => {
	...
	const [stats, setStats] = useState({ totalBaseWords: 0, totalVariants: 0, pendingVariants: 0, approvedVariants: 0 });
	...
	const [variantsFilter, setVariantsFilter] = useState<"all" | "approved" | "pending">("all");
	...
	const fetchStats = async () => {
		const [{ count: bwCount }, { count: vCount, data: variantsData }] = await Promise.all([
			supabase.from("base_words").select("*", { count: "exact", head: true }),
			supabase.from("lexicon_entries").select("status", { count: "exact" })
		]);
		
		const approved = variantsData?.filter(v => v.status === "approved").length || 0;
		const pending = variantsData?.filter(v => v.status === "pending").length || 0;
	...
	const fetchBaseWords = async () => {
		...
		const { data, error } = await supabase.from("base_words").select("*").order("word", { ascending: true });
		if (!error && data) {
			setBaseWords(data);
	...
	const handleApprove = async (id: string) => {
		const { error } = await supabase.from("lexicon_entries").update({ status: "approved" }).eq("id", id);
		if (error) { toast.error("Error approving entry"); return; }
		setVariants(curr => curr.map(v => v.id === id ? { ...v, status: "approved" } : v));
	...
	const handleDeleteVariant = async (id: string) => {
		...
		if (error) { toast.error("Error deleting entry"); return; }
		setVariants(curr => curr.filter(v => v.id !== id));
		toast.success("Variant deleted");
	};

	const handleDeleteBaseWord = async (id: string) => {
		if (!confirm("Are you sure you want to delete this base word? All related variants may also be removed.")) return;
		const { error } = await supabase.from("base_words").delete().eq("id", id);
		if (error) { toast.error("Error deleting base word"); return; }
		...
		<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
			<div className='flex items-center space-x-4 mb-12'>
				...
				<div>
					...
					<p className='text-brand-ink/60 font-medium'>Manage all lexicon data</p>
				</div>
			</div>
							...
							<div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
								<div className='glass-card p-6 rounded-2xl border border-brand-ink/5 bg-brand-orange text-white'>
									<div className='flex flex-col items-center justify-center text-center'>
										...
										<span className='text-4xl font-bold font-serif mb-1'>{stats.totalBaseWords}</span>
										<span className='text-sm font-bold uppercase tracking-widest opacity-80'>Total Base Words</span>
									</div>
								</div>
								<div className='glass-card p-6 rounded-2xl border border-brand-ink/5'>
									<div className='flex flex-col items-center justify-center text-center'>
										<Layers size={32} className='mb-2 text-brand-orange/80' />
										<span className='text-4xl font-bold font-serif mb-1'>{stats.totalVariants}</span>
										<span className='text-sm font-bold uppercase tracking-widest text-brand-ink/40'>Total Variants</span>
									</div>
								</div>
							</div>
							...
							<div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
								<div className='glass-card p-6 rounded-2xl border border-brand-ink/5'>
									<div className='flex flex-col items-center justify-center text-center'>
										...
										<span className='text-2xl font-bold font-serif mb-1'>{stats.approvedVariants}</span>
										<span className='text-xs font-bold uppercase tracking-widest text-brand-ink/40'>Approved Variants</span>
									</div>
								</div>
								<div className='glass-card p-6 rounded-2xl border border-brand-ink/5'>
									<div className='flex flex-col items-center justify-center text-center'>
										<Clock size={24} className='text-brand-orange mb-2' />
										<span className='text-2xl font-bold font-serif mb-1'>{stats.pendingVariants}</span>
										<span className='text-xs font-bold uppercase tracking-widest text-brand-ink/40'>Pending Variants</span>
									</div>
								</div>
							</div>
						</motion.div>
					...
					{activeTab === "base-words" && (
						<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className='space-y-6'>
                            <div className="flex justify-between items-center mb-6">
                                <h2 className='text-3xl font-serif font-bold'>Manage Base Words</h2>
                            </div>
								...
								{loadingBaseWords ? (
									...
									<div className="flex flex-col">
										<ul className="divide-y divide-brand-ink/5 max-h-[60vh] overflow-y-auto">
											{baseWords.slice(0, visibleBaseWords).map(bw => (
												<li key={bw.id} className="p-4 flex justify-between items-center group">
													<div>
														<span className="font-serif font-bold text-lg">{bw.word}</span> <span className="text-sm text-brand-ink/40">({bw.syllables} syllables)</span>
														{bw.note && <p className="text-xs text-brand-ink/40 line-clamp-1">{bw.note}</p>}
													</div>
													<div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
														<button className="text-brand-ink/40 hover:text-brand-orange cursor-pointer" onClick={() => setEditingBaseWordId(bw.id)}><Edit3 size={18} /></button>
														<button className="text-brand-ink/40 hover:text-red-500 cursor-pointer" onClick={() => handleDeleteBaseWord(bw.id)}><Trash2 size={18} /></button>
													</div>
												</li>
											))}
										</ul>
										{visibleBaseWords < baseWords.length && (
											<div className="p-4 border-t border-brand-ink/5 bg-gray-50/50">
												<button 
													onClick={() => setVisibleBaseWords(prev => prev + LOAD_MORE_COUNT)}
									...
									<div className="py-12 text-center text-brand-ink/40">No base words found.</div>
								)}
							</div>
						</motion.div>
					)}
							...
							{loadingVariants ? (
								...
								<div className="flex flex-col">
									<div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
										{variants
											.filter(entry => variantsFilter === "all" ? true : entry.status === variantsFilter)
											...
											<AdminLexiconCard
												...
												onEdit={() => setEditingVariantId(entry.id)}
												onDelete={handleDeleteVariant}
											/>
										))}
									</div>
									{visibleVariants < variants.filter(entry => variantsFilter === "all" ? true : entry.status === variantsFilter).length && (
										<button 
											onClick={() => setVisibleVariants(prev => prev + LOAD_MORE_COUNT)}
