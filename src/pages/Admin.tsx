...
export const Admin: React.FC = () => {
  ...
  const filteredEntries = entries.filter(e => {
    ...
    const matchesSearch = e.word.toLowerCase().includes(searchTerm.toLowerCase()) || 
  ...
  if (isAdmin === false) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        ...
        <p className="text-brand-ink/60 mb-8">You do not have administrative privileges to access this page.</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 space-y-6 md:space-y-0">
        <div>
          <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4">Admin Console</h1>
          <p className="text-brand-ink/60 text-lg">Review, edit, and approve community contributions.</p>
        </div>
      ...
      <div className="space-y-6">
              ...
              <div className="flex flex-col lg:flex-row justify-between gap-6">
                <div className="flex-grow">
                  <div className="flex items-center space-x-3 mb-2">
                    <h3 className="text-2xl font-serif font-bold">{entry.word}</h3>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                      entry.status === 'approved' ? 'bg-green-100 text-green-600' : 'bg-brand-orange/10 text-brand-orange'
                    }`}>
                      {entry.status}
                    </span>
                  </div>
        ...
        {filteredEntries.length === 0 && (
          <div className="py-24 text-center border-2 border-dashed border-brand-ink/5 rounded-3xl">
            <Filter size={48} className="mx-auto text-brand-ink/10 mb-6" />
            <h3 className="text-2xl font-serif font-bold mb-2">No entries found</h3>
            <p className="text-brand-ink/60">Try adjusting your filters or search terms.</p>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-brand-ink/5 flex justify-between items-center">
                <h3 className="text-2xl font-serif font-bold">Edit Entry</h3>
                <button 
                  onClick={() => setEditingEntry(null)}
              ...
              <form onSubmit={handleUpdate} className="p-8 max-h-[70vh] overflow-y-auto space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingEntry({...editingEntry, example_yoruba: e.target.value})}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingEntry({...editingEntry, example_english: e.target.value})}
                  />
                </div>
