...
import { Plus, Clock, CheckCircle, AlertCircle, LogOut, User, Trash2, Edit3, Save, X } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 space-y-6 md:space-y-0">
        <div>
          <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4">Dashboard</h1>
          <div className="flex items-center space-x-4 text-brand-ink/60">
            ...
            <button 
              ...
              className="flex items-center space-x-2 text-red-500 hover:text-red-600 transition-colors text-sm font-bold uppercase tracking-widest"
            >
              <LogOut size={16} />
              <span>Sign Out</span>
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="glass-card p-8 rounded-3xl border border-brand-ink/5 shadow-xl">
            ...
            <div className="space-y-6">
              ...
              />
              <StatItem 
                icon={<AlertCircle size={20} className="text-brand-ink/20" />}
          ...
          <div className="p-8 rounded-3xl bg-brand-orange text-white">
            <h3 className="text-xl font-serif font-bold mb-4">Contributor Guide</h3>
            <p className="text-white/80 text-sm leading-relaxed mb-6">
              Ensure your entries follow the standard Yorùbá orthography. Include diacritics where necessary to maintain phonetic accuracy.
            </p>
        ...
        <div className="lg:col-span-2">
          <h3 className="text-2xl font-serif font-bold mb-8">Recent Contributions</h3>
          <div className="space-y-6">
            <AnimatePresence mode="popLayout">
              {loading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="h-32 rounded-2xl bg-white/30 animate-pulse border border-brand-ink/5" />
                ))
                  ...
                  >
                    <div>
                      <div className="flex items-center space-x-3 mb-2">
                        <h4 className="text-xl font-serif font-bold">{contribution.word}</h4>
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="text-right mr-4 hidden md:block">
                        <div className="text-[10px] font-bold uppercase tracking-widest text-brand-ink/30">Submitted</div>
                        <div className="text-xs font-medium">{new Date(contribution.created_at).toLocaleDateString()}</div>
                      </div>
                      <button 
                ...
                <div className="py-24 text-center border-2 border-dashed border-brand-ink/5 rounded-3xl">
                  <Plus size={48} className="mx-auto text-brand-ink/10 mb-6" />
                  <h3 className="text-2xl font-serif font-bold mb-2">No contributions yet</h3>
                  <p className="text-brand-ink/60 mb-8">Start by adding your first word to the archive.</p>
                  <button 
                    onClick={() => setIsAdding(true)}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-brand-ink/5 flex justify-between items-center">
                <h3 className="text-2xl font-serif font-bold">New Lexicon Entry</h3>
                <button 
                  onClick={() => setIsAdding(false)}
              ...
              <form onSubmit={handleSubmit} className="p-8 max-h-[70vh] overflow-y-auto space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 ml-1">Word</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 ml-1">Phonetic</label>
                    <input
                      type="text"
                      className="input-field"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 ml-1">Part of Speech</label>
                  <select 
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 ml-1">Definition</label>
                  <textarea
                    rows={3}
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 ml-1">Example (Yorùbá)</label>
                  <input
                    type="text"
                    className="input-field"
                    placeholder="Use the word in a sentence..."
                    value={newEntry.example_yoruba}
                    onChange={(e) => setNewEntry({...newEntry, example_yoruba: e.target.value})}
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-brand-ink/40 ml-1">Example (English Translation)</label>
                  <input
                    type="text"
                    className="input-field"
                    placeholder="Provide the English translation..."
                    value={newEntry.example_english}
                    onChange={(e) => setNewEntry({...newEntry, example_english: e.target.value})}
                  />
                </div>
